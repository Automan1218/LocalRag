...
public class JwtUtils {
    ...
    private static final long REFRESH_TOKEN_EXPIRATION_TIME = 7 * 24 * 60 * 60 * 1000; // 24 * 7 hours
    ...
    public String generateToken(String username){
        ...
        // 添加组织标签信息
        if(user.getOrgTags()!=null && !user.getPrimaryOrg().isEmpty()){
    ...
    private String extractTokenIdFromToken(String token) {
        try{
            Claims claims = extractClaimsIgnoreExpiration(token);
